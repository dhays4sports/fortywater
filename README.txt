CURRENT HUMAN TRUST RELEASE: 408-UI-3.13.1E — Human Trust Regression + Production Certification
HUMAN TRUST STATUS: COMPLETE / DEPLOYABLE-CERTIFIED / UI FROZEN
GOVERNING PRINCIPLE: Modern system underneath. Human relationship on the surface.
LIFE: explicitly excluded from Human Trust styling and preserved in cinematic campaign mode
FINAL HUMAN TRUST CERTIFICATION: HUMAN-TRUST-PRODUCTION-CERTIFICATION.md
POST-DEPLOY REQUIREMENT: UI3_13_1E_DEPLOYMENT_SMOKE_RUNBOOK.md
FOUNDATION DOC: HUMAN-TRUST-DESIGN-SYSTEM.md

CURRENT UI RELEASE: 408-UI-3.13 — Production Design Certification
UI-3 STATUS: COMPLETE / DEPLOYABLE-CERTIFIED
CERTIFIED BASELINE: 408-UI-3.13 + 408-INFRA-1.1 + 408-UI-3.2.1
INFRA BOUNDARY: 408-INFRA-1.1 `_routes.json` preserved and certified 22/22
BEHAVIOR BASELINE: 408-UI-3.12 end-to-end regression remains authoritative
CAMPAIGN PRESENTATION: 408-UI-3.11.1 message-match registry remains authoritative
POST-DEPLOY REQUIREMENT: run deployed-domain/external-service/Cloudflare invocation smoke after this ZIP is deployed; build-time certification cannot prove a deployment that has not occurred.

Production-design certification artifacts:
- SPRINT-408-UI-3.13.md
- PRODUCTION-DESIGN-CERTIFICATION.md
- UI3_13_RELEASE_CERTIFICATION.json
- UI3_13_BROWSER_QA.json
- UI3_13_METADATA_QA.json
- UI3_13_ASSET_PERFORMANCE_QA.json
- UI3_13_FREEZE_QA.json
- UI3_13_DEPLOYMENT_SMOKE_RUNBOOK.md
- SPRINT-408-INFRA-1.1.md
- INFRA1_1_RELEASE_CERTIFICATION.json

CURRENT LOCAL CERTIFICATION: 408-LOCAL-1.10 — Conversion + Compliance Certification

Technical/source certification passes. Production activation for the roadmap's full three-merchant MVP remains NO-GO because Auto + Home merchants, physical merchant/device closeout, and agency/Farmers advertising approval are still external prerequisites. Stevie's remains the only active real merchant. No synthetic businesses were added.

Run: `node qa/test-408-local-1.10.js`
Browser QA: `python qa/test-408-local-1.10-browser.py`
External GO checklist: `LOCAL1_10_EXTERNAL_CLOSEOUT.md`
Compliance preflight: `LOCAL1_10_COMPLIANCE_PREFLIGHT.md`
Release certification: `LOCAL1_10_RELEASE_CERTIFICATION.json`
QA snapshot: 146/146 Local certification, 210/210 rendered-browser checks, 3/3 QR decode, 430/430 public internal links, 37/37 public/runtime JavaScript syntax, 296/296 static regression.
Browser note: deployed-domain navigation is blocked inside the certification sandbox, so rendered-browser QA uses the real production HTML/CSS and generated Local markup through Playwright `page.set_content`; live deployed-domain smoke testing remains external.
Detailed roadmap: `408-LOCAL-ROADMAP.md`

Previous Local release candidate: **408-LOCAL-1.9 — Pilot Merchant Launch**

Previous Local release: **408-LOCAL-1.8 — 408FARMERS Site Integration**

Previous Local release: **408-LOCAL-1.7 — Insurance Conversion Bridge**

408FARMERS Local adds the optional post-value homeowner insurance bridge on active-perk merchant pages while keeping merchant redemption independent. It reuses the 1.6 attribution engine and existing Home / Home + Auto journeys without adding a Local lead form or automatically launching CoverageFit.

Run: `node qa/test-408-local-1.7.js` (historical release harness; current 1.8 advances Local page build markers and roadmap state)

Previous Local release: **408-LOCAL-1.6 — Local Attribution Engine**

408FARMERS Local preserves bounded non-PII merchant/perk/surface attribution, emits the five certified Local events, and can carry Local origin into a later voluntary insurance request without a duplicate lead form. `/api/local/event` provides optional durable D1 persistence and never blocks directory browsing or perk redemption.

Run: `node qa/test-408-local-1.6.js` (historical release harness; current 1.8 advances Local page build markers)
Worker QA: `node qa/test-408-local-1.6-worker.mjs`
Detailed roadmap: `408-LOCAL-ROADMAP.md`
Historical next sprint from 1.6: `408-LOCAL-1.7 — Insurance Conversion Bridge`
...
Public route: /local/join/
Purpose: repeatable pilot application for South Bay merchants, with business/contact/category/location/proposed-perk fields and explicit authorization + insurance-separation acknowledgments.
Delivery: POST /api/local/merchant-application -> Formspree, with optional LOCAL_MERCHANT_FORMSPREE_ENDPOINT override and direct HTML-form fallback.
No application automatically publishes a merchant, requires commercial insurance, guarantees directory acceptance, or promises referrals/lead volume/sales.
Branded receipt: /local/join/thank-you.html
Detailed continuation roadmap: 408-LOCAL-ROADMAP.md
Next sprint: 408-LOCAL-1.6 — Local Attribution Engine

408-LOCAL-1.3 — MERCHANT DISCOVERY DIRECTORY
Public route: /local/
Purpose: data-driven South Bay merchant discovery with category filtering, featured presentation, lifecycle-safe perk states, responsive controls, and useful empty/error states.
Runtime: shared/local-directory.js consumes the validated 408-LOCAL-1.2 catalog/model.
Production fixtures remain draft/non-public, so no real merchant or offer claim is introduced by this release.
No login, consumer lead form, insurance gate, redemption flow, merchant detail route, Local attribution, or insurance conversion bridge is introduced in 1.3.
Detailed continuation roadmap: 408-LOCAL-ROADMAP.md
Next sprint: 408-LOCAL-1.4 — Merchant Perk Detail + Redemption

408-LOCAL-1.2 — MERCHANT DATA MODEL
Public route: /local/ (superseded visually by 408-LOCAL-1.3 directory UI)
Purpose: versioned, reusable source-of-truth for future South Bay Local merchants and merchant-owned perks.
Canonical data: local/data/catalog.json
Schema: local/data/catalog.schema.json
Runtime model: shared/local-data-model.js
The model provides stable merchant/perk IDs, canonical `/local/{slug}/` URL construction, lifecycle/date gating, independent-offer language enforcement, no-endorsement guardrails, and storage-neutral structure for a later Cloudflare-backed store.
Three fixture merchants (Eat & Drink, Home, Auto) remain draft/non-public and do not represent live businesses or offers.
No merchant directory, redemption engine, Local attribution, insurance conversion bridge, or real merchant activation is introduced in 1.2.
Detailed continuation roadmap: 408-LOCAL-ROADMAP.md
Next sprint at time of 1.2: 408-LOCAL-1.3 — Merchant Discovery Directory

408-LOCAL-1.7 — INSURANCE CONVERSION BRIDGE
Merchant detail route: /local/{merchant-slug}/
Purpose: optional homeowner conversion only after merchant value has been delivered.
Bridge: Home + Auto review or Home-only review using the existing 408FARMERS journeys and certified Local attribution continuity.
No merchant perk requires an insurance quote, lead, policy purchase, or CoverageFit assessment. The dedicated bridge is not rendered on merchant pages without an active perk.
No duplicate Local lead form and no automatic CoverageFit launch are introduced.
Detailed continuation roadmap: 408-LOCAL-ROADMAP.md
Next sprint: 408-LOCAL-1.8 — 408FARMERS Site Integration


408-LOCAL-1.9 pilot state: Stevie's is the first real active Local merchant; Auto/Home pilot recruitment remains an external closeout gate. See SPRINT-408-LOCAL-1.9.md and local/pilot/PILOT-LAUNCH-RUNBOOK.md.
