...
(() => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const nativeFormspreeFallback = () => {
      try {
        form.setAttribute('action', directEndpoint || DIRECT_FORMSPREE_ENDPOINT);
    ...
    try {
      ...
      try {
        ...
        if (form.dataset.coveragefitAfterSubmit === 'true' && window.CoverageFitLauncher) {
          window.CoverageFitLauncher.launch({
            ...
            next: form.dataset.cfNext || '/assessment/',
            fallbackUrl: form.dataset.success || 'thank-you.html',
            extra: { lead_captured: 'true', lead_capture_status: 'confirmed', submitted_at: submittedAt }
          });
        } else {
          location.href = form.dataset.success || 'thank-you.html';
        }
      ...
      status.textContent = 'We could not confirm the online submission. Tap once more to use the direct secure Formspree submission.';

      // A second click bypasses every JavaScript transport and performs a
      // standards-based HTML POST directly to Formspree.
      button.dataset.nativeFallback = 'true';
      button.addEventListener('click', function directFallback(event) {
        event.preventDefault();
        button.disabled = true;
        label.textContent = 'Opening Secure Submission…';
        if (!nativeFormspreeFallback()) {
          button.disabled = false;
          label.textContent = original;
