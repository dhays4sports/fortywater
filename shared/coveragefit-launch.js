(function (window, document) {
  ...
  var PASSTHROUGH_KEYS = [
    'source', 'partner_id', 'perk_id', 'merchant_slug', 'surface', 'variant',
  ...
  function getAttribution() {
    ...
    // 408-LOCAL-1.6: if the visitor previously arrived through Local and later
    // reaches an insurance form without query parameters (for example through
    // the homepage), restore only the bounded non-PII Local origin context.
    // Any explicit campaign/source query on the current page wins instead.
    var localFallbackValues = {};
    if (localStorage && Object.keys(explicitValues).length === 0) {
      try {
        var localRecord = JSON.parse(localStorage.getItem('408farmers_local_attribution_v1') || 'null');
        if (localRecord && localRecord.schema_version === '408-local-attribution-v1' && Number(localRecord.expires_at) > Date.now() && localRecord.context && localRecord.context.source === 'local') {
          PASSTHROUGH_KEYS.forEach(function (key) {
            var value = localRecord.context[key];
            if (value) {
              attribution[key] = value;
              localFallbackValues[key] = value;
            }
          });
        }
      } catch (error) {}
    ...
    if (localStorage && (Object.keys(explicitValues).length || Object.keys(localFallbackValues).length)) {
      var merged = Object.assign({}, storedUtm, localFallbackValues, explicitValues);
      try { localStorage.setItem(DEFAULTS.utmStorageKey, JSON.stringify(merged)); } catch (error) {}
      var selectedCampaign = explicitValues.campaign || localFallbackValues.campaign;
      if (selectedCampaign) {
        try { localStorage.setItem(DEFAULTS.campaignStorageKey, selectedCampaign); } catch (error) {}
      }
    }
  ...
  function buildUrl(options) {
    ...
    // Preserve Local as the truthful acquisition source when a visitor voluntarily
    // enters insurance from 408FARMERS Local. Other legacy source values continue
    // to use the established 408FARMERS sender source.
    url.searchParams.set('source', attribution.source === 'local' ? 'local' : config.source);
    ...
    PASSTHROUGH_KEYS.forEach(function (key) {
      if (key === 'campaign' || key === 'source') return;
      if (key === 'referral') {
