(function (window, document) {
  ...
  var reviewContextSelect = form.elements.review_context;
  ...
  function setReviewContextVisible(visible) {
    if (!reviewContext) return;
    reviewContext.hidden = !visible;
    if (visible) reviewContext.style.setProperty('display', 'block', 'important');
    else reviewContext.style.removeProperty('display');
    if (reviewContextSelect) reviewContextSelect.required = Boolean(visible);
  }
  ...
  function configureContext(housingContext, formFirst) {
    ...
    setReviewContextVisible(Boolean(formFirst));
    if (renter) {
      propertyFields.hidden = true;
      propertyAddress.required = false;
      setReviewContextVisible(false);
      if (reviewContextSelect && !reviewContextSelect.value) reviewContextSelect.value = 'Need a new policy';
